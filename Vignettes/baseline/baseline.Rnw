...
trace (spc.fit.poly.below, exit = quote ({
...
plot (chondro [3,,1705 ~ 1742], lines.args = list(type = "b"), add=T)
cl <- "black"
pch = 1
...
\caption{Influence of \Rcode{fit.to} on the baseline polynomial. The black baseline is fit to the
  spectral range \rcm{1700 - 1750}, the blue to \rcm{1720 - 1750} only (dots \& circles: supporting
  points).}
...
<<trace2, echo = FALSE, results = hide>>=
	trace (spc.fit.poly.below, exit = quote ({ls ();
...
\section{Convenience functions \Rcode{spc.rm.poly.below} and \Rcode{spc.rm.rubberband}}
In a normal use case we don't need the baseline itself, but the corrected spectrum. So there
are two convenience functions that save some typing. They calculate baselines
as described above and return already corrected spectra.
<< results = hide >>=
# Two lines below give identical results
paracetamol - spc.fit.poly.below(paracetamol[,, 400~2000], paracetamol)
spc.rm.poly.below(paracetamol, wl.range = c(400~2000))
@
<< results = hide >>=
# Two lines below give identical results
paracetamol - spc.rubberband(paracetamol)
spc.rm.rubberband(paracetamol)
@
