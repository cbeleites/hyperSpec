...
% \VignetteKeywords{hyperSpec, import, export}
...
\warnbox{Reproducing the Examples in this Vignette}{The source code of this vignette including the
  spectra files are available as .zip file at \phy's home page:
  \url{http://hyperspec.r-forge.r-project.org/blob/fileio.zip}
...
options (warn = 1)
if (!require ("testthat") || !require ("digest")){
  test_file <- function (...) cat ("tests skipped: packages testthat and/or digest missing.\n")
  test_dir <- test_file
}
@
<<testthat-setup, echo = FALSE, results=hide>>=
...
see section \ref{sec:scan.txt.Renishaw} on p.~\pageref{sec:scan.txt.Renishaw}). Also, both
...
Witec also saves ASCII data with spectra in columns (Export $\rightarrow$ Table), see \ref{sec:scan.txt.Witec}.
...
## not covered:
# setdiff (c (Sys.glob ("jcamp-dx/*"), Sys.glob ("jcamp-dx/PE-IR/*")), files)

## exclude files already checked with the visible examples: 
...
for (file in files [! files %in% unsupported]) {
  cat (file)
  read.jdx (file, ytol = 1e-6)
  cat ("\n")
}
@

\warnbox{Note}{\Rfunction{read.jdx.Shimadzu} is deprecated.}
...
if (colnames != c ("Intensity", "Wavelength (&Aring;)", "Spectrum", "Ref. "))
...
\warnbox{\Rcode{read.cytomat} is \textbf{deprecated}.}{%
...
Agilent (Varian) uses a variant of ENVI (with binary header). A specialized form of read.ENVI will be
coming soon.
...
%
An optimized import function for the ASCII files is available: \Rfunction{scan.txt.Renishaw}. The
file may be compressed via gzip, bzip2, xz or lzma. zip compressed files are read via
\Rfunction{scan.zip.Renishaw}.  The ASCII files can easily become very large, particularly with
linefocus- or streamline imaging.  \Rfunction{scan.txt.Renishaw} provides two mechanisms to avoid
running out of memory during data import. The file may be imported in chunks of a given number of
lines (see the last example). \Rfunction{scan.txt.Renishaw} can calculate the correct number of
...
\emph{Therefore it is not possible to import multi-spectra files with individually ``zapped'' spectra
  using \Rfunction{scan.txt.Renishaw}.}
...
Instead of a file name, \Rfunction{scan.txt.Renishaw} accepts also a connection.
...
<<scan.txt.Renishaw-compressed, results = hide>>=
...
spc <- read.txt.Horiba.xy ("txt.HoribaJobinYvon/map.txt", remove.zerospc = FALSE)
if (any (dim (spc) != c (398, 3, 616)) || 
    is.null (spc$x) || any (is.na (spc$x)) || 
    is.null (spc$y) || any (is.na (spc$y)) ||
    length (setdiff (wl (spc), 1 : 616)) == 0L)
  stop ("error in testing read.txt.Horiba.xy. Please contact ", maintainer ("hyperSpec"))

spc <- read.txt.Horiba.t ("txt.HoribaJobinYvon/ts.txt", remove.zerospc = FALSE)
if (any (dim (spc) != c (100, 2, 1024)) || 
...
\label{sec:scan.asc.Andor}
\index{Andor Solis!ASCII}
\index{ASCII!Andor Solis}
\index{Raman!Andor Solis!ASCII} 

Andor Solis exports ASCII files that can be read with \Rfunction{scan.asc.Andor}:
<<>>=
scan.asc.Andor("asc.Andor/ASCII-Andor-Solis.asc")
@

\subsection{Witec}
\label{sec:scan.txt.Witec}
\index{Witec!ASCII}
\index{ASCII!Witec}
...
Imaging data (but also single spectra and time series) can be exported as ASCII X and Y files (Save ASCII X and Save ASCII Y, not supported in version 4). These can be read by \Rfunction{scan.dat.Witec}:
<<witec-dat, results=hide>>=
scan.dat.Witec ("txt.Witec/Witec-timeseries-x.dat")
scan.dat.Witec (filex = "txt.Witec/Witec-Map-x.dat", 
...
Another option is Witec's txt table ASCII export (Export $\rightarrow$ Table), which produces ASCII files with each row corresponding to one wavelength. The first column contains the wavelength axis, all further columns contain one spectrum each column. Such files can be read with \Rfunction{scan.txt.Witec}:
<<witec-txt, results=hide>>=
scan.txt.Witec ("txt.Witec/Witec-timeseries_no.txt")
@ 
\Rfunction{scan.txt.Witec} determines the number of wavelengths automaticallly. 
...
scan.txt.Witec ("txt.Witec/Witec-Map_full.txt", type = "map", hdr.label = TRUE, hdr.units = TRUE)

scan.txt.Witec ("txt.Witec/Witec-Map_label.txt", type = "map", hdr.label = TRUE, hdr.units = FALSE)

scan.txt.Witec ("txt.Witec/Witec-Map_unit.txt", type = "map", hdr.label = FALSE, hdr.units = TRUE)

scan.txt.Witec ("txt.Witec/Witec-Map_unit.txt", type = "map", hdr.label = FALSE, hdr.units = TRUE, 
                points.per.line = 5)

scan.txt.Witec ("txt.Witec/Witec-Map_no.txt", type = "map", hdr.label = FALSE, hdr.units = FALSE)
...
general enough to work immediately with new data: the the file header is completely ignored. Thus
...
<<catch-missing-tests, echo=FALSE>>=
