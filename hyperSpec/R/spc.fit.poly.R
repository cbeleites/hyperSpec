...
.test (spc.fit.poly) <- function (){
  ...
  test_that("un-normalized large wavelengths with higher order polynomials throw error", {
    expect_error (spc.fit.poly(tmp, poly.order = 3, offset.wl = FALSE))
  })

  test_that("normalized large wavelengths with higher order polynomials", {
...
.test (spc.fit.poly.below) <- function (){
  ...
  bl.nonorm <- spc.fit.poly.below (flu, flu, poly.order = 3, offset.wl = FALSE, npts.min = 12)
  ...
  # was issue 1 on github
  test_that("un-normalized large wavelengths with higher order polynomials throw error", {
    expect_error (spc.fit.poly.below (tmp, poly.order = 3, offset.wl = FALSE, npts.min = 12))
  })

  test_that("normalized large wavelengths with higher order polynomials", {
    bl.1e4 <- spc.fit.poly.below (tmp, tmp, poly.order = 3, offset.wl = TRUE, npts.min = 12)
    expect_equal (bl.nonorm [[]], bl.1e4 [[]])
  })
}
