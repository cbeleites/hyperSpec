.all_equal <- function(target, current, ..., check_attributes = FALSE,
                       check_names = FALSE,
                        check_column_order = FALSE, check_label = FALSE,
      tolerance = hy_getOption ("tolerance"), 
      wl_tolerance = hy_getOption("wl.tolerance")) {
  validObject(target)
  validObject(current)
  tolerance    <- .checkpos(tolerance,"tolerance")
  wl_tolerance <- .checkpos(wl.tolerance, "wl.tolerance")
...
.test(.all.equal) <- function() {
  ...
  test_that("column order", {
    ...
    expect_true(! isTRUE(all.equal(flu, flu [, rev(colnames(flu))],
                                check.column.order = TRUE)))
    expect_true(all.equal(flu, flu [, rev(colnames(flu))], 
...
##'   equality
##' @param ... handed to \code{\link[base]{all.equal}} when testing 
##' the slots of the
...
##' @param tolerance,wl.tolerance tolerances for checking wavelengths
##'  and data, respectively
##' @return \code{all.equal} returns either \code{TRUE},
