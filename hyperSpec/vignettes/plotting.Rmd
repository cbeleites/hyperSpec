...
Besides, package **tripack**[`r cite_pkg("tripack", bib = pkg_bib)`] is mentioned, but not used in this vignette.
...
**`levelplot()`{.r}**
:  package **hyperSpec** has a method for package **lattice**[`r cite_pkg("lattice", bib = pkg_bib)`] function `levelplot()`{.r}.
...
If available, the package package **plotrix**`[r cite_pkg("plotrix", bib = pkg_bib)`] is used to produce the cut mark.
...
```{r plotmap-chondro, fig.cap=CAPTION}
plotmap(chondro)
```
...
Voronoi uses `panel.voronoi()`{.r} from package **latticeExtra**[`r cite_pkg("latticeExtra", bib = pkg_bib)`].
The tesselation is calculated by default using package **deldir**[`r cite_pkg("deldir", bib = pkg_bib)`], but package **tripack**[`r cite_pkg("tripack", bib = pkg_bib)`] can also be used.
...
Package **rgl**[`r cite_pkg("rgl", bib = pkg_bib)`] offers fast 3d plotting in R.
...
Class `hyperSpec`{.r} objects do not yet directly support plotting with package **ggplot2**[`r cite_pkg("ggplot2", bib = pkg_bib)`].
