...
```{r child = "about-vignettes.Rmd", echo = FALSE}
```
...
In addition, [chondro](#abt-vigs), [laser](#abt-vigs) and [flu](#abt-vigs) are accompanied by their own vignettes showing sample work flows for the respective data type.
...
For details see the [plotting](#abt-vigs) vignette.
...
\mFun{`as.t.df()`{.r}}
The transpose of a wide format `data.frame`{.r} can be obtained by `as.t.df()`{.r}. For further examples, see the discussion of package **ggplot2** in the [plotting](#abt-vigs) vignette.
...
These are discussed in detail in a separate [fileio](#abt-vigs) vignette.  Here we just mention a couple of basic operations.
...
More information about converting existing data into `hyperSpec`{.r} objects can be found in the [fileio](#abt-vigs) vignette.
...
For details, see [baseline](#abt-vigs) vignette.
...
See the [plotting](#abt-vigs) vignette.
...
`file.remove.emptyspc` | `TRUE`{.r} | Automatic removing of empty spectra                         | File import functions, see [fileio](#abt-vigs)
`file.keep.name`       | `TRUE`{.r} | Automatic recording of file name in column `filename`{.r}   | File import functions, see [fileio](#abt-vigs)
