...
```{r}
faux_cell <- faux_cell - spc.fit.poly.below(faux_cell[, , min + 3i ~ max - 3i], faux_cell)
faux_cell <- sweep(faux_cell, 1, rowMeans(faux_cell[[]], na.rm = TRUE), "/")
```
...
```{r normalize1}
faux_cell <- sweep(faux_cell, 1, mean, "/")
```
...
faux_cell <- sweep(faux_cell, 1, factors, "*")
```
...
```{r norm-2}
faux_cell <- faux_cell * factors
```

Put together, this results in:

```{r areanorm-shortcut}
faux_cell <- faux_cell / rowMeans(faux_cell[, , 1600 ~ 1700])
```
...
faux_cell <- scale(faux_cell, center = quantile(faux_cell, 0.05), scale = FALSE)
plot(faux_cell, "spcprctl5")
```
