---
# For bookdown ---------------------------------------------------------------
bookdown_file_name: hyperspec
bookdown_titles: |
  # (PART\*) I Principles {-}
  # `hyperspec`: Introduction to Package **_hyperSpec_** {#hyperspec}
# For vignette ---------------------------------------------------------------
...
**V. Gegzna's notes** `hyperspec-1`

1. I suggest keeping tags `\\mFun{}` and `\\index{}`.
They are not useful for HTML format yet, but in the future we can come up with useful ideas how to create an index of functions.
But they are ignored by HTML rendering engine (as they are LaTeX tags) and removed from the output.

2. I general, it seems that this tutorial could be split into several smaller tutorials:
    a. I think, there should be one additional short tutorial "Quick-start".
    b. I suggest moving sections such as `Spectral (Pre)processing`, `Data Analysis` to separate vignettes.

3. Some ideas should be more clearly explained for beginner-level users without assuming that they know technical details.
```

```{block, type = "note-t", echo = show_reviewers_notes}
**B. Hansons's notes** `Some remaining issues`

There is still a lot of work and polishing to be done.  Here are some of the remaining organizational and document processing issues as I see it:

1. I think we can greatly simplify the bibliography processing by just letting `markdown` take care of it.

2. For all vignettes we should probably settle on one or two color schemes for plots.  At least some are defined in `vignette-functions.R`.

3.  I'm not sure if any of the `\index ` and related items do anything in this type of document.  Remove if possible.  html files can be search via the browser.  Note: **VG wants to keep for now.**

4.  Some maintenance simplification might be gained by putting the contents of all tables into data frames and using `kableExtra` to print them.
...
**`fauxCell`**      \index{data sets!fauxCell}
                    A synthetic data set similar to `chondro`.
...
In addition, `chondro`, `laser` and `flue` are accompanied by their own vignettes showing sample work flows for the respective data type.
...
```{block, type = "note-t", echo = show_reviewers_notes}
**V. Gegzna's notes** `hyperspec-6`

1. The output of print() and summary() is identical. Is it expected?
I think, the `print()`{.r} method should give less details than it does now.
```



\mFun{print, show, summary}
As usual, the `print()`{.r}, `show()`{.r} and `summary()`{.r} methods provide an overview of the object (only `print` is shown here):
...
```{block, type = "note-t", echo = show_reviewers_notes}
**B. Hansons's notes** `This is not clear`

This line: _Function `colnames<-`{.r} renames the labels as well._ is not clear and the documentation doesn't seem to say anything about it.
```
...
* I consolidated all the various parts dealing with getters and setters here.
* In the tables, I tried to be systematic and put in every combination of getters and setters.  In the process I discovered some that don't work.  These should be fixed if they are logical operations, or intercepted if not and the user warned. The setters table has some open questions that need to be investigated.
* This material would be good in a cheatsheet... eventually.
```
<!-- ======================================================================= -->


The main functions to retrieve the data of a `hyperSpec`{.r} object are `[]` and `[[]]`. \mFun{`[]`,`[[]]`}
The difference between these functions is that `[]` returns a `hyperSpec`{.r} object, whereas the result of `[[]]` is a `data.frame`{.r}.
**hyperSpec** objects are triple indexed:

* `x[i, j, l]`{.r}
* `x[[i, j, l]]`{.r}
* `i` refers to rows of the `@data`{.r} slot.
* `j` refers to columns of the `@data`{.r} slot.
* `l` refers to wavelengths.
...
## Access Functions for **_hyperSpec_** Objects {#sec:fast-access-parts}

Table \@ref(tab:getters) shows the main functions that can be used with class `hyperSpec`{.r} to access the object ("getters").  Table \@ref(tab:setters) shows the main functions that can be used with class `hyperSpec`{.r} to modify the object ("setters").

<!-- ---------------------------------------------------------------------- -->
: **(\#tab:getters)** Getter functions for the slots of `hyperSpec`{.r} objects
...
**`x[, j, ]`{.r}**       **returns damaged object -- investigate!**
**`x[, , l]`{.r}**       Returns the `hyperSpec` object with selected wavelengths.
**`x[[]]`{.r}**          Returns the spectra matrix (`x$spc`{.r}).
**`x[[i, , ]]`{.r}**     Returns the spectra matrix (`x$spc`{.r}) with selected rows.
**`x[[, j, ]]`{.r}**     Returns a `data.frame`{.r} with the selected columns.
**`x[[, , l]]`{.r}**     Returns the spectra matrix (`x$spc`{.r}) with selected wavelengths.
...
<!-- ---------------------------------------------------------------------- -->
: **(\#tab:setters)** Setter functions for the slots of `hyperSpec`{.r} objects

------------------------ -----------------------------------------------------------------------------------------------------------------------
**`x[i, ,] <-`{.r}**     Replaces the specified rows.
**`x[, j,] <-`{.r}**     Replaces the specified columns.
**`x[, , l] <-`{.r}**    Replace the specified wavelengths.
**`x[[i, ,]] <-`{.r}**   **does this make sense? -- investigate!**
**`x[[, j,]] <-`{.r}**   **does this make sense? -- investigate!**
**`x[[, , l]] <-`{.r}**  **does this make sense? -- investigate!**
...
**`wl<-`{.r}**           Set the wavelength vector.
**`labels<-`{.r}**       Set the labels.
------------------------ -----------------------------------------------------------------------------------------------------------------------
<!-- ---------------------------------------------------------------------- -->
...
\mFun{`wl2i()`{.r} `i2wl()`{.r}}
Spectra in package **hyperSpec** always have discretized wavelength axes, stored in a matrix with each column corresponding to one wavelength.
...
These are discussed in detail in a separate vignette accessible via `vignette("fileio")`.  Here we just mention a couple of basic operations.
...
**`data`**       Any extra data as a `data.frame`.  It is possible to pass `spc` via this argument if the data frame has a column `$spc`{.r}.
...
```{block, type = "note-t", echo = show_reviewers_notes}
**B. Hansons's notes** `Clarifications`

1. Need more explanation of the pooled covariance approach.
2. Here's a place that we should standardize the color scheme.
3. Why not generate `rnd` only once here, and use it for the plot and LDA?  As it stands now it seemed like the first set of spectra were somehow related to the 2nd set.
```

\mFun{rmmvnorm}
If package **mvtnorm**[`r cite_pkg(pkg_bib, "mvtnorm")`] is available, multivariate normally distributed spectra can be generated from mean and covariance matrix using `rmmvnorm()`{.r} (Fig. \@ref(fig:sim-spc)).
...
Fig. \@ref(fig:sim-lda) shows the linear discriminant analysis (LDA) scores of simulated spectra in comparison to the real spectra in the `chondro`{.r} object:
...
<!-- ======================================================================= -->
```{block, type = "note-t", echo = show_reviewers_notes}
**V. Gegzna's notes** `hyperspec-10`


1. Empty chapter. What to do with it?

Comment from Roman: I suggest for now to write the following:

Correction of cosmic spikes is not a part of `hyperSpec` package, and can be addressed with algorithms presented in these papers:
...
Package package **baseline**[`r cite_pkg(pkg_bib, "baseline")`] offers many more functions for baseline correction.
...
\mFun{`pls::msc()`{.r}}
Multiplicative scatter correction (MSC) can be done using `msc()`{.r} from package package **pls** [`r cite_pkg(pkg_bib, "pls")`].
...
# Appendix 1. Overview of the functions provided by package **hyperSpec** {-}



```{r tab-fn, results = "asis", echo = FALSE, eval = FALSE}
make.fn.table <- function() {
  load("intro--functions.RData")
...
<!-- ======================================================================= -->
```{block, type = "note-t", echo = show_reviewers_notes}
**V. Gegzna's note** `hyperspec-12`

`# TODO`{.r}  A better version of the table could be created if `pkgdown` website
 (documentation) is used. The index/list of functions could be created automatically.
...
# Appendix 3. Reporting Bugs and Getting Help {-}

```{block, type= "note-t", echo = show_reviewers_notes}
**B. Hanson's notes**

* This material probably belongs elsewhere.
* Should also add something about using StackOverflow etc.
```

## Reporting an issue

`R` packages include contact information of the package maintainer, which you can access e.g., by:
...
`bug.report()`{.r} will either open an e-mail to the package maintainer or the issue tracker URL given in the package `DESCRIPTION`.
...
You can also make suggestions in this manner.
...
# Appendix 4. Speed and Memory Considerations {- #sec:speed-considerations}
...
Package **matrixStats**[`r cite_pkg(pkg_bib, "matrixStats")`] implements fast functions to calculate summary statistics for each row  or each column of a matrix.

## Memory usage

In general, it is not recommended to work with variables that are more than approximately a third of the available RAM in size. Particularly the import of raw spectroscopic data can consume large amounts of memory.
...
The same behaviour is triggered in `read.ENVI()`{.r} and its derivatives (`read.ENVI.<it>Manufacturer</it>()`{.r}).
The memory consumption of `read.txt.Renishaw()`{.r} can be lowered by importing the data in chunks (argument `nlines`{.r}).

# Appendix 5. Remarks on R, or How **_hyperSpec_** Works Under the Hood {-}

```{block, type= "note-t", echo = show_reviewers_notes}
**B. Hanson's notes**

* This is more of an explanation of how R works, I don't think it is needed here at all.
```

`hyperSpec`{.r} objects are defined using an S4 class. Much of **hyperSpec** is devoted to manipulating these objects using the methods instrinsic to `R`.
...
Package **hyperSpec** implements a variety of such functions, see Table \@ref(tab:functions).
