...
# Combining and Splitting `hyperSpec`{.r} Objects {#sec:combine}
...
Depending on the type of analysis, one may wish to replace the `NA`{.r}s by interpolating the neighbour values. Package **hyperSpec** provides three functions that can interpolate the `NA`{.r}s: `spc.NA.approx()`{.r}, `spc.loess()`{.r}, and `spc.bin()`{.r} with `na.rm = TRUE`{.r} (the latter two are discussed below).
...
The results of such a \mFun{`decomposition()`{.r}} decomposition can be put back into `hyperSpec`{.r} objects.
...
\mFun{`[[]]`{.r}}
Some `R` functions expect their input data in a matrix, including those related to HCA. Conveniently, the `[[]]`{.r} extraction function returns a matrix.

```{r hca}
dist <- pearson.dist(chondro[[]])
```

Again, many such functions coerce the data to a matrix automatically, so the `hyperSpec`{.r} object can be handed over without the explicit conversion above (as we saw for PCA):
