...
The difference between these functions is that `[]` returns a `hyperSpec`{.r} object, whereas `[[]]` returns a `data.frame` containing `x$spc`{.r}, the spectral data.
...
**`x[[i, , l, wl.index = TRUE/FALSE]] <-`{.r}** Replaces the intensity values in `x$spc`{.r} for the specified wavelengths in the specified rows.
