...
```{r child = "about-vignettes.md", echo = FALSE}
```
...
In addition, [`chondro`](#list-of-vignettes), [`laser`](#list-of-vignettes) and [`flu`](#list-of-vignettes) are accompanied by their own vignettes showing sample work flows for the respective data type.
...
\mFun{`as.t.df()`{.r}}
The transpose of a wide format `data.frame`{.r} can be obtained by `as.t.df()`{.r}. For further examples, see the discussion of package **ggplot2** in the [plotting](#list-of-vignettes) vignette.
...
These are discussed in detail in a separate [fileio](#list-of-vignettes) vignette.  Here we just mention a couple of basic operations.
...
For the `faux_cell`{.r} data, let's use the maximum of the peak between around 1200 cm^-1^.
...
bandpos <- apply(faux_cell[[, , 1190 ~ 1210]], 1, find.max, wl(faux_cell[, , 1190 ~ 1210]))
refpos <- find.max(colMeans(faux_cell[[, , 1190 ~ 1210]]),  wl(faux_cell[, , 1190 ~ 1210]))
...
```{r}
faux_cell.tmp <- faux_cell - spc.fit.poly.below(faux_cell[, , min + 3i ~ max - 3i], faux_cell)
faux_cell.tmp <- sweep(faux_cell.tmp, 1, rowMeans(faux_cell[[]], na.rm = TRUE), "/")
```
...
```{r eval = FALSE}
ir.spc   <- faux_cell / 1500 ## fake IR data
...
faux_cell.offset.corrected <- sweep(faux_cell, 1, offsets, "-")
```
...
```{r ofs2}
faux_cell.offset.corrected <- sweep(faux_cell, 1, min, "-")
```
...
```{r normalize1}
faux_cell.tmp <- sweep(faux_cell, 1, mean, "/")
```
...
faux_cell.tmp <- sweep(faux_cell, 1, factors, "*")
```
...
```{r norm-2}
faux_cell.tmp <- faux_cell * factors
```

Put together, this results in:

```{r areanorm-shortcut}
faux_cell.tmp <- faux_cell / rowMeans(faux_cell[, , 1600 ~ 1700])
```
...
faux_cell.tmp <- scale(faux_cell, center = quantile(faux_cell, 0.05), scale = FALSE)
plot(faux_cell.tmp, "spcprctl5")
```
...
```{r msc, eval = FALSE}
library(pls)
faux_cell.msc <- faux_cell
faux_cell.msc[[]] <- msc(faux_cell[[]])
```
...
This kind of noise reduction may influence further modeling of the data. Mathematically speaking, the rank of the new `r nrow (faux_cell)` $×$ `r nwl (faux_cell)` spectra matrix is only 10.
...
plotmap(faux_cell, clusters ~ x * y, col.regions = cluster.cols)
```
...
```{r merge-sample}
faux_cell.low <- sample(faux_cell[, , min ~ 1200], 700)
nrow(faux_cell.low)
```
```{r merge-sample-2}
faux_cell.high <- sample(faux_cell[, , 1400 ~ max], 700)
nrow(faux_cell.high)
```
...
```{r merge}
faux_cell.merged <- merge(faux_cell.low, faux_cell.high)
nrow(faux_cell.merged)
```
...
```{r}
faux_cell.merged <- merge(faux_cell.low, faux_cell.high, all = TRUE)
nrow(faux_cell.merged)
```
...
plot(faux_cell.merged[1:100], "mat", col = matcols)
```
