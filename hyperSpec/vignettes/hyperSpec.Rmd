...
\mFun{rmmvnorm}
If package **mvtnorm**[`r cite_pkg("mvtnorm", bib = pkg_bib)`] is available, multivariate normally distributed spectra can be generated from mean and covariance matrix using `rmmvnorm()`{.r} (Fig. \@ref(fig:sim-spc)).
...
Package package **baseline**[`r cite_pkg("baseline", bib = pkg_bib)`] offers many more functions for baseline correction.
...
\mFun{`pls::msc()`{.r}}
Multiplicative scatter correction (MSC) can be done using `msc()`{.r} from package package **pls** [`r cite_pkg("pls", bib = pkg_bib)`].
...
Package **matrixStats**[`r cite_pkg("matrixStats", bib = pkg_bib)`] implements fast functions to calculate summary statistics for each row  or each column of a matrix.
