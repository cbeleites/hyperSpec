---
# For bookdown ---------------------------------------------------------------
# `baseline`: Fitting Baselines to Spectra {#baseline}
# For vignette ---------------------------------------------------------------
...
```{r setup-baseline, include=FALSE}
# Packages -------------------------------------------------------------------
library(hyperSpec)
...
#   fig.path   = "resources/figures/baseline--"
# )
```
...
Package **hyperSpec** provides two fitting functions for polynomial baselines, `spc.fit.poly()`{.r} and `spc.fit.poly.below()`{.r}. 
Another possibility is `spc.rubberband()`{.r}, a "rubberband" method that determines support points by finding the convex hull of each spectrum.
...
The main difference between the two functions is that `spc.fit.poly()`{.r} returns a least squares fit through the complete spectrum that is given in `fit.to`{.r} whereas `spc.fit.poly.below()`{.r} tries to find appropriate spectral regions to fit the baseline to.
...
```{r classical, fig.cap=CAPTION, fig.width=8, fig.height=4}
...
Both `spc.fit.poly()`{.r} and `spc.fit.poly.below()`{.r} therefore provide least squares fitting for the polynomial.

Function `spc.fit.poly()`{.r} fits to the whole spectral region of `fit.to`{.r}.
...
Function `spc.fit.poly.below()`{.r} tries to automatically find appropriate spectral regions for baseline fitting. 
...
The baselines and respective supporting points for each iteration of `spc.fit.poly.below(chondro[1], poly.order = 1)`{.r} are shown in figure \@ref(fig:iter).
...
```
<!-- fig1 -->
```{r iter, fig.cap=CAPTION, results='hide', message=FALSE, warning=FALSE, fig.width=12, fig.height=4}
...
```
<!-- figspcrange -->
```{r spectralrange, fig.cap=CAPTION, pdf=TRUE, results='hide', eps=FALSE, fig.width=8, fig.height=4}
plot(chondro[3, , 1700 ~ 1750],
  lines.args = list(type = "b"),
  plot.args = list(ylim = range(chondro[3, , 1700 ~ 1750]) + c(-50, 0))
)
...
This selection of appropriate spectral regions is essential for `spc.fit.poly()`{.r}. 
But also `spc.fit.poly.below()`{.r} can benefit from narrower spectral ranges: the fitting gains speed.
...
```{r include=FALSE}
CAPTION <- "Baseline polynomial fit to the first spectrum of the chondro data set of order 0 -- 2 (left to right).
The dots indicate the points used for the fitting of the polynomial.  "
```

```{r figorder0, fig.cap=CAPTION, pdf=TRUE, results='hide', eps=FALSE, fig.width=5, fig.height=4}
...
```{r figorder1, fig.cap=CAPTION, pdf=TRUE, results='hide', eps=FALSE, fig.width=5, fig.height=4}
...
```{r figorder2, fig.cap=CAPTION, pdf=TRUE, results='hide', eps=FALSE, fig.width=5, fig.height=4}
bl <- spc.fit.poly.below(chondro[1], poly.order = 2, debuglevel = 2, max.iter = 5)
```

Figures \@ref(fig:figorder0), \@ref(fig:figorder1), \@ref(fig:figorder2) show the resulting baseline polynomial of `spc.fit.poly.below (chondro [1], poly.order = order)`{.r} with `order`{.r} $=$ 0 to 3 for the first spectrum of the chondro data set.
...
```{r fig3, fig.cap=CAPTION, results='hide', message=FALSE, fig.width=7.5, fig.height=3}
...
```{r fig4, fig.cap=CAPTION, results='hide', message=FALSE, fig.width=7.5, fig.height=4}
...
The resulting baselines for `spc.fit.poly.below (chondro [1], poly.order = 1, noise = 12)`{.r} of the whole spectrum are shown in the middle and lower part of figure \@ref(fig:fig4).
...
```{r rubberband-raw, fig.cap=CAPTION, fig.width=6, fig.height=4, results='hide', include=TRUE}
...
```{r rubberband, fig.cap=CAPTION, fig.width=6, fig.height=4, results='hide', include=TRUE}
...
```{r rubberband-bent-raw, fig.cap=CAPTION, fig.width=6, fig.height=4, results='hide', include=TRUE}
...
```
<!-- rubberband-bent-corrected -->
```{r rubberband-bent-corrected, fig.cap=CAPTION, fig.width=6, fig.height=4, results='hide', include=TRUE}
plot(paracetamol[, , 175 ~ 1800] - bl)
```
