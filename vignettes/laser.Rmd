---
# For bookdown ---------------------------------------------------------------
# `laser`: Unstable Laser Emission {#laser}
# For vignette ---------------------------------------------------------------
title:       Unstable Laser Emission
subtitle:    Vignette for the Data Set `laser` of the R package **hyperSpec**
# subtitle:  Vignette on hyperSpec's laser data set
description: "laser: Tutorial on hyperSpec's `laser` data set."
# Authors --------------------------------------------------------------------
author:
...
```{r setup-laser, include=FALSE}
# Packages -------------------------------------------------------------------
library(hyperSpec)
# library(rgl)
```

```{r setup-laser-2, include=FALSE}
# Functions ------------------------------------------------------------------
source("resources/vignette-functions.R", encoding = "UTF-8")
```

```{r setup-laser-3, include=FALSE}
# Settings -------------------------------------------------------------------
source("resources/vignette-default-settingsR.R", encoding = "UTF-8")

# knitr::opts_chunk$set(
#   fig.path   = "resources/figures/laser--"
# )
```
...
```{r}
file  <- "resources/fileio/txt.Renishaw/laser.txt.gz"
laser <- read.txt.Renishaw(file, data = "ts")
```

```{r include=FALSE}
CAPTION <- "The raw spectra laser emission spectra.  "
```

```{r rawspc, fig.cap=CAPTION, fig.width=5, fig.height=3}
...
```{r cutspc, fig.cap=CAPTION, fig.width=5, fig.height=3}
laser <- laser[, , -75 ~ 0]
plot(laser, "spcprctl5")
```
...
```{r wlspc, fig.cap=CAPTION, fig.width=5, fig.height=3}
plot(laser, "spcprctl5")
```
...
<!-- ======================================================================= -->
```{block, type="note-t", echo=TRUE}
**Translator's notes** `laser-3`


1. `# NOTE:`{.r} It's not clear, why the data is saved at this point.
...
```{r markspc, fig.cap=CAPTION, fig.width=5, fig.height=3}
...
```{r ts, fig.cap=CAPTION, fig.width=10, fig.height=3}
...
```{r tsextra, fig.cap=CAPTION, fig.height=3, fig.width=8}
...
Class `hyperSpec`{.r} objects  can easily be drawn with package **rgl**`r cite_pkg("rgl")`:
...
```{r rgl-do, echo=FALSE, eval=FALSE, results='hide'}
# ============================================================================
...
if (require(rgl)) {
  ...
  png("resources/laser-fig-3D.png")
  ploterrormsg("", "rgl")
  dev.off()
}
```
...
knitr::include_graphics("resources/laser-rgl-3d.png")
```
