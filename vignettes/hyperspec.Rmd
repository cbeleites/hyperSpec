...
2. For all vignettes we should probably settle on one or two color schemes for plots.

3.  I'm not sure if any of the `\index ` and related items do anything in this type of document.  Remove if possible.  html files can be search via the browser.
...
5.  I think `vignette.defs` which is basically a \LaTeX file is not doing anything for this document.
```
...
# Things to Know About **_hyperSpec_** As You Get Started
...
abbreviation for *"column xxx of the `data.frame`{.r} in slot data of the object"* (the structure of `hyperSpec`{.r} objects is discussed in section \@ref(sec:structure-chy)).

## The Structure of `hyperSpec`{.r} Objects {#sec:structure-chy}

`hyperSpec`{.r} objects are defined using an S4 class. It contains the following slots:
...
```{r structure, echo=FALSE, fig.cap=CAPTION, out.width="400"}
knitr::include_graphics("resources/intro--strukturhyperspec.png")
CAPTION = "The structure of the data in a `hyperSpec` object.  In this example the 'extra data' are the `x` and `y` columns in `@data`."
```
...
A complete list of the user-facing functions that are available in package **hyperSpec** is given in Table \@ref(tab:functions).

## Launching **_hyperSpec_** {#sec:loading-package}
...
make.fn.table <- function() {
  load("resources/intro--functions.RData")
